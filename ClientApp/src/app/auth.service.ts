...
import { apiRoute, tokenGetter } from './constants';
...
export class AuthService {
  ...
  authenticate(data?: any) {
    const result = this.http.request('post', `${apiRoute}/identity/login`, { body: data, observe: 'response' })
      .pipe(
        map((res) => {
  ...
  register(data?: any) {
    ...
    const result =  this.http.request('post', url, { body: data, observe: 'response' })
      .pipe(
        map((res) => {
          return new AuthResult(
            true,
